<div class="" id="top-section">
    <div class="text-2xl sm:text-4xl lg:text-5xl my-60 w-4/5 heading">
        <h1>Empower Learning with <strong>certify</strong> – Create, Test, Certify!</h1>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
        <div class="col-span-1 paragraph">
            <p>Whether you're training your team or sharing knowledge with the world, certify makes it easy to create engaging courses, design interactive tests, and award professional certificates. Streamline your training process or grow your audience with our user-friendly tools. Start building your course today and empower learners to succeed!</p>
        </div>

        <div class="col-span-1 md:col-span-2">
            <img class="image w-full" src="{{ asset('landingpage-images/top-section.jpg') }}" alt="">
        </div>
    </div>
</div>